...
%token ASSIGNOP PLUS MINUS STAR DIV AND OR DOT NOT STRUCT RETURN BREAK CONTINUE IF ELSE WHILE FOR SELFADD SELFDEC ADD_ASSIGNOP MINUS_ASSIGNOP STAR_ASSIGNOP DIV_ASSIGNOP
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
        ...
        | VarDec SELFADD   {$$=mknode(1,SELFADD,yylineno,$1);strcpy($$->type_id,"SELFADD");}
        | SELFADD VarDec   {$$=mknode(1,SELFADD,yylineno,$2);strcpy($$->type_id,"SELFADD");}
        | VarDec SELFDEC   {$$=mknode(1,SELFDEC,yylineno,$1);strcpy($$->type_id,"SELFDEC");}
        | SELFDEC VarDec   {$$=mknode(1,SELFDEC,yylineno,$2);strcpy($$->type_id,"SELFDEC");}
        | VarDec ADD_ASSIGNOP Exp  {$$=mknode(2,ADD_ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ADD_ASSIGNOP");}
        | VarDec MINUS_ASSIGNOP Exp {$$=mknode(2,MINUS_ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"MINUS_ASSIGNOP");}
        | VarDec STAR_ASSIGNOP Exp  {$$=mknode(2,STAR_ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"STAR_ASSIGNOP");}
        | VarDec DIV_ASSIGNOP Exp   {$$=mknode(2,DIV_ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"DIV_ASSIGNOP");}
        | VarDec {$$ = $1;}
